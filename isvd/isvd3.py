...
class ISVD3():
    ...
    def fit(self, M_int: (np.ndarray)) -> None:
        ...
        uavg_cond = np.linalg.cond(self.U_avg)

        if uavg_cond > self.condition_threshold or self.U_avg.shape[0] != self.U_avg.shape[1]:
            U_avg_inv = np.linalg.pinv(self.U_avg)
        else:
            U_avg_inv = np.linalg.inv(self.U_avg)
